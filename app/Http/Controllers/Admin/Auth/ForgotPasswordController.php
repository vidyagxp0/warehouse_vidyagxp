...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function verifyCode(Request $request)
    {
        ...
        $resetToken = AdminPasswordReset::where('status', Status::ENABLE)->where('email',session()->get('pass_res_mail'))->latest('id')->first();

        if ($resetToken->token != $request->code) {
            $notify[] = ['error', 'Verification code mismatchh'];
            return to_route('admin.password.reset')->withNotify($notify);
        }
